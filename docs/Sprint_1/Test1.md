...
| **M2**|`Slicing&BusinessLogic` | Der Brotlänge L sei 300.0 (float). Die Scheibendicke T sei 10.0 (float). | Rufe calculateSliceCount(300.0, 10.0) auf. | Die zurückgegebene Anzahl der Scheiben N ist 30 (int). |keine Zustandsänderung|
| **M3** | `LengthSensor` | Das Modul ist initialisiert und simuliert eine feste Länge von **$L_{max}=200.0$**. | Aufruf der Methode `measureLength()`. | Rückgabe des simulierten Werts von **200.0**. | keine Zustandsänderung|
